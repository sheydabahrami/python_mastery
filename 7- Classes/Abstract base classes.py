...
class Stream(ABC): # Deriving the Stream() classes from the ABC() class we make it abstract
    def __init__(self):
        self.opened = False 
...
# There are a couple of issues with this impelementation
# The first issue is that we can creat a Stream object and call the "open()"" method

#stream = Stream()
#stream.open()

# Why is this an issue. Because this stream class is an abstract. What are we working with a Network, File?
# We should not be able to directly creat an instance of the Stream() class.
