...
class InvalidOperationError(BaseException): # Creatining a costum error. By convention all costum exceptions should en with "Error". And every time we create a costum exception in a class, we should derive the exception from the Base exeption.
    pass


class Stream: # Here we are defining a class with the comom fetures between the streams, like open and close.
    def __init__(self):
        self.opened = False # Create a constructor and set an opened flag initialy to false.
...
# We dont't have multy level inheritance
